...
public class SemiFinalRecyclerAdapter extends RecyclerView.Adapter<SemiFinalRecyclerAdapter.ViewHolder> {

    private final List<FaseFinal> semiList;

    public SemiFinalRecyclerAdapter(final List<FaseFinal> semiList) {
        this.semiList = semiList;
    }
    ...
    @Override
    public SemiFinalRecyclerAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.linha_semi, parent, false);
        return new ViewHolder(v);

    }

    @Override
    public void onBindViewHolder(SemiFinalRecyclerAdapter.ViewHolder linhaViewHolder , int position) {

        FaseFinal linha = semiList.get(position);
